$(() => {
	...
	const logoAnimation = () => {
		...
		tlTyping.addCallback(() => logoBottom.addClass('animate'), '+=3').staggerTo($('#logo-bottom > span'), 0, { display: 'inline' }, .15, '+=1.5').staggerTo($('#logo-bottom>b>span'), 0, { display: 'inline' }, .15, '+=1.5').addCallback(() => tlIntro.to(glimpse, .5, { y: '0%' }), '+=2').addCallback(() => window.location.replace('about.html'), '+=.5');
	};
