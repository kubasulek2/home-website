...
$(document).ready(function () {
	...
	if ($('body#projects').length && mqDesktop.matches) {
		...
		$('.row').each(function (i) {
			...
			const
				triggerOut = $(this).parent().find('.project-out')[0],
			...
			projectsIn
				.setTween(TweenMax.staggerFrom( projects, 1,{opacity: 0, ease: Power2.easeIn, cycle: {x:[-1000,1000]}},0))
				.addTo(controller);
			
			if (i < $('.row').length - 1 ) projectsOut
				.setTween(TweenMax.to( projects, 1,{opacity: 0, ease: Power2.easeOut}))
				.addTo(controller);	
